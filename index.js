...
Lexer.prototype = {
  ...
  error: function(code, message) {
    ...
    var err = console.error(code, message, JSON.stringify(data));
    throw err;
  },
  ...
  isExpression: function(exp) {
    return isExpression(exp, {throw: true});
  },
  ...
  addText: function(type, value, prefix, escaped) {
    if (value + prefix === '')
      return;
    prefix = prefix || '';
    var indexOfEnd = this.interpolated
      ? value.indexOf(']')
      : -1;
    ...
    if (indexOfStart !== Infinity && indexOfStart < indexOfEnd && indexOfStart < indexOfEscaped && indexOfStart < indexOfStringInterp) {
      ...
      this.tokens.push(this.tok('start-hmap-interpolation'));
      this.incrementColumn(2);
      ...
      this.tokens.push(this.tok('end-hmap-interpolation'));
      this.incrementColumn(1);
  ...
  each: function() {
    ...
    if (captures = /^- *(?:each|for) +([a-zA-Z_$][\w$]*)(?: *, *([a-zA-Z_$][\w$]*))? +in +([^\n]+)/.exec(this.input)) {
      this.error('MALFORMED_EACH', 'Hmap each and for should no longer be prefixed with a dash ("-"). They are hmap keywords and not part of JavaScript.');
    }
  },
    ...
    if ('(' == this.input.charAt(0)) {
      ...
      var isEndOfAttribute = function(i) {
        ...
        if (loc === 'key') {
          if (whitespaceRe.test(str[i])) {
            ...
            for (var x = i; x < str.length; x++) {
              if (!whitespaceRe.test(str[x])) {
                // starts a `value`
                if (str[x] === '=' || str[x] === '!')
                  return false; // will be handled when x === i
                else if (str[x] === ',')
                  return false; // attribute ended
                else
                  return true;
                }
              }
          ...
          if (!self.assertExpression(val, true))
            return false;
          if (whitespaceRe.test(str[i])) {
            // find the first non-whitespace character
            for (var x = i; x < str.length; x++) {
